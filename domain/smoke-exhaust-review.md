...
| 關鍵字 | 類型 | 折減係數 |
|--------|------|----------|
| casement, 平開, 側開, pivot, 樞軸, tilt, 內倒內開 | 推開／平開窗 | 1.0（工具預設） |
| sliding, 橫拉, 推拉, hung, 上下拉 | 橫拉窗 | 0.5（整樘面積基準） |
| awning, 上懸, 外推, hopper, 下懸, projected | 推射／外推／上懸窗 | 1.0（工具預設） |
| louver, 百葉 | 百葉 | 0.5 |
...
排煙檢核公式：`Required Area = Room Area × 2%`。**Room Area 用哪個值會直接影響合規判定**。

日本 BIM 模板（及部分台灣模板）的 Room 元素**同時保存兩個面積值**：
- **面積**（Area）：Revit 自動計算（幾何面積）—— `check_smoke_exhaust_windows` 用的是這個
- **面積 部屋 調整値**：建模者手填校正值（仕上表 / 法定報告用）

**5/22 dry-run 實測**：店舗 26 兩值差 1.00 m² → Required Area 差 0.02 m²。在 2% 邊界 case（接近合格門檻）上可能跨越判定結果。

**AI 應對策略**：報告排煙 PASS/FAIL 時，**主動 surface 兩個面積值並存**：「依工具回的『面積』為 X m²，須補 Y m²。若改用『部屋調整値』Z m²，須補 W m²。哪個值是合規檢討基準需由人決定。」

詳見 `domain/lessons.md` L-029、`domain/daylight-area-check.md` 末段。
